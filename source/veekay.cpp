...
int veekay::run(const veekay::ApplicationInfo& app_info) {
	...
	*/
	...
	{ // NOTE: Initialize Vulkan: grab device and create swapchain
        ...
        // [FIX] Включаем Dynamic Rendering для Vulkan 1.2
        ...
        auto selector_result = physical_device_selector.set_surface(vk_surface)
                                                       ...
                                                       .add_required_extension("VK_KHR_dynamic_rendering") // Обязательно расширение
                                                       .add_required_extension_features(dynamic_rendering_feature) // Обязательно включаем фичу
                                                       .select();
		if (!selector_result) {
